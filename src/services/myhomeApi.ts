...
async function loadApiKeys() {
  try {
    console.log('🔍 MyHome: Attempting to load API keys from agencies copy.json...');
    const response = await fetch('/agencies copy.json');
    if (response.ok) {
      apiKeys = await response.json();
      console.log('✅ MyHome: Loaded API keys from agencies copy.json:', apiKeys.length, 'entries');
      console.log('📋 MyHome: First entry structure:', apiKeys[0]);
      console.log('📋 MyHome: Available SitePrefixes:', apiKeys.map(k => k.SitePrefix || k.sitePrefix || 'NO_PREFIX'));
    } else {
      console.warn('⚠️ MyHome: Could not load agencies copy.json - HTTP', response.status);
    }
  } catch (error) {
    console.error('❌ MyHome: Error loading API keys from agencies copy.json:', error);
    
    // Try alternative file names
    try {
      console.log('🔍 MyHome: Trying agency-keys.json as fallback...');
      const fallbackResponse = await fetch('/agency-keys.json');
      if (fallbackResponse.ok) {
        apiKeys = await fallbackResponse.json();
        console.log('✅ MyHome: Loaded API keys from agency-keys.json fallback:', apiKeys.length, 'entries');
      }
    } catch (fallbackError) {
      console.error('❌ MyHome: Fallback also failed:', fallbackError);
    }
  }
}
